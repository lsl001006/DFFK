...
def accuracy(output, target, topk=(1,)):
    ...
    if len(target.shape) == 2:  # multi label
...
def extract_state_dict():
    dataset = "cifar10"
    teacher = "wrn40_2"
    torch.save(torch.load('ckpt/pretrained/%s_%s.pth' % (dataset, teacher), map_location='cpu')['state_dict'],
               "ckpt/fed/cifar10/a1.0+sd1+e500+b16/0.pt")
