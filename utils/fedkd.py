...
class myFed:
    def __init__(self, student, teacher, generator, discriminator, writer, args):
        self.writer = writer
        self.args = args
        self.val_loader, self.priv_data, self.priv_ood_data, self.local_datanum, self.local_cls_datanum = build_dataloader(
            args)
        ...
        if not self.args.logfile == 'debug':
            for n in range(config.N_PARTIES):
                ...
                if os.path.exists(savename):
                    ...
                    tr_dataset = cifar.Dataset_fromarray(self.priv_data[n]['x'], self.priv_data[n]['y'], train=True,
                                                         verbose=False)
                    train_loader = DataLoader(
                        dataset=tr_dataset, batch_size=config.BATCHSIZE, shuffle=True, num_workers=config.NUM_WORKERS,
                        sampler=None)
    ...
    def init_training(self):
        ...
        for n in range(config.N_PARTIES):
            ...
            ood_dataset = cifar.Dataset_fromarray(self.priv_ood_data[n]['x'], self.priv_ood_data[n]['y'], train=True,
                                                  verbose=False)
            local_ood_loader = DataLoader(
                dataset=ood_dataset, batch_size=config.DIS_BATCHSIZE, shuffle=True, num_workers=config.NUM_WORKERS,
                sampler=None)
        ...
        self.sched_s = optim.lr_scheduler.CosineAnnealingLR(
            self.optim_s, steps_all, eta_min=config.DIS_LR_MIN, )
        # for gen
